...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        mine_action (state, choice1, choice2, currentPlayer, handPos);
    ...
    case baron:
        return baron_action (state, choice1, currentPlayer);
    case great_hall:
        //+1 Card
        drawCard(currentPlayer, state);
    ...
    case minion:
        minion_action(state, choice1, choice2, handPos, currentPlayer);
    ...
    case tribute:
        tribute_action(state, nextPlayer, tributeRevealedCards, currentPlayer);
    case ambassador:
        ambassador_action(state, choice1, choice2, handPos, currentPlayer);
    case cutpurse:
...
int baron_action (struct gameState *state, int choice1, int currentPlayer) {
        ...
        if (choice1 > 0) { //Boolean true or going to discard an estate
            ...
            while(card_not_discarded) {
                if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
                    state->coins += 10;//Add 4 coins to the amount of coins
    ...
    int minion_action (struct gameState *state, int choice1, int choice2, int handPos, int currentPlayer) {
        //+1 action
            state->numActions++;
            int i, j;
            //discard card from hand
...
int ambassador_action (struct gameState *state, int choice1, int choice2, int handPos, int currentPlayer) {
    int j;
...
int mine_action (struct gameState* state, int choice1, int choice2, int currentPlayer, int handPos){
    int i;
    int j;
    i = 0;
    j = 0;
    j = state->hand[currentPlayer][choice1];  //store card we will trash
        ...
        if (state->hand[currentPlayer][choice1] > gold || state->hand[currentPlayer][choice1] < copper)
        {
            return -1;
        }
...
int tribute_action (struct gameState*  state, int nextPlayer, int *tributeRevealedCards, int currentPlayer) {
    int i;
    i = 0;
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
            ...
            else if (state->discardCount[currentPlayer] > 0) {
